...
目测优化 3 轮（已做完：chunk_size、搜索上限、滑动窗口）
    ↓
集成 RAGAS 评估 pipeline，出 baseline 分数（进行中）
    ↓
边评估边继续迭代，用分数验证效果
    ↓
收敛后进入 V4
...
20 条 query 全量评估结果（F=Faithfulness, R=Answer Relevancy）：

| # | Query | F | R | 可见问题 |
|---|-------|---|---|---------|
| 1 | V3 主要做什么？ | 0.143 | 0.736 | 检索命中标题碎片，缺少具体描述 |
| 2 | chunk_text vs recursive_split | 0.333 | 0.908 | 线索 |
| 3 | Agent run 方法参数 | 0.000 | 0.805 | 上下文完全无关 |
| 4 | 避免切中断句 | 0.270 | 0.995 | 上下文碎片化 |
| 5 | 向量库存放目录 | 0.000 | 0.874 | 答案准确但无可信上下文 |
| 6 | 为什么选 Chroma | 0.114 | 0.000 | 评分类似失败 |
| 7 | V2 改了哪些文件 | 0.509 | 0.863 | ✅ |
| 8 | 为什么不用 LangChain | 0.500 | 0.827 | ✅ |
| 9 | 为什么选 MiniLM | 0.789 | 0.839 | ✅ |
| 10 | 多用户支持 | 0.958 | 0.912 | ✅ |
| 11 | Docker 方案 | 0.176 | 0.752 | Plan 信息零散未聚集 |
| 12 | VectorStore 核心方法 | 0.600 | 0.919 | ✅ |
| 13 | ReAct 循环流程 | 0.083 | 0.907 | 上下文无实际代码/流程图 |
| 14 | RAG Tool save/search | 0.000 | 0.896 | 上下文均无实际代码 |
| 15 | Chunker/VectorStore 调用 | 1.000 | 0.846 | ✅ 完美命中 |
| 16 | 文档上传到检索步骤 | 0.231 | 0.811 | 上下文零散 |
| 17 | 大文件处理 | 0.154 | 0.920 | 仅 chunking 相关上下文 |
| 18 | chunk_size/overlap 影响 | 0.650 | 0.895 | ✅ |
| 19 | 异常体系可重试类 | 0.091 | 0.803 | 上下文仅有"Phase 1"标题 |
| 20 | Streamlit 瘦客户端通信 | 1.000 | 0.868 | ✅ 完美命中 |

**结论**: Faithfulness 低不是回答错误，而是检索上下文不足以支撑回答。LLM 用自身知识补全细节后被 RAGAS 判为"不忠实于上下文"。核心瓶颈是 chunk 碎片化 + embedding 质量。
