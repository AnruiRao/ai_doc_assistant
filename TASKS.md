...
20 条 query 全量评估结果（F=Faithfulness, R=Answer Relevancy），bge-base-zh-v1.5 + chunk 合并：

| # | Query | F | R | 可见问题 |
|---|-------|---|---|---------|
| 1 | V3 主要做什么？ | 0.625 | 0.890 | ✅ 大幅改善 |
| 2 | chunk_text vs recursive_split | 0.911 | 0.982 | ✅ 显著提升 |
| 3 | Agent run 方法参数 | 1.000 | 0.948 | ✅ 完美命中 |
| 4 | 避免切中断句 | 0.440 | 0.805 | 🔶 仍需改善 |
| 5 | 向量库存放目录 | 0.750 | 0.644 | ✅ 改善 |
| 6 | 为什么选 Chroma | 1.000 | 0.880 | ✅ 完美命中 |
| 7 | V2 改了哪些文件 | 0.451 | 0.901 | 🔶 检索仍散 |
| 8 | 为什么不用 LangChain | 0.913 | 0.969 | ✅ 显著提升 |
| 9 | 为什么选 MiniLM | 0.905 | 0.905 | ✅ 保持高位 |
| 10 | 多用户支持 | 0.697 | 0.834 | ↔ 基本持平 |
| 11 | Docker 方案 | 0.682 | 0.882 | ✅ 大幅改善 |
| 12 | VectorStore 核心方法 | 0.289 | 0.956 | 🔶 列举型问题 |
| 13 | ReAct 循环流程 | 0.778 | 0.937 | ✅ 大幅改善 |
| 14 | RAG Tool save/search | 0.393 | 0.959 | 🔶 列举型 |
| 15 | Chunker/VectorStore 调用 | 0.862 | 0.860 | ✅ 改善 |
| 16 | 文档上传到检索步骤 | 0.500 | 0.839 | ✅ 改善 |
| 17 | 大文件处理 | 0.306 | 0.831 | 🔶 假设型 |
| 18 | chunk_size/overlap 影响 | 0.177 | 0.920 | 🔶 分析型 |
| 19 | 异常体系可重试类 | 0.194 | 0.813 | 🔶 列举型 |
| 20 | Streamlit 瘦客户端通信 | 0.833 | 0.883 | ✅ 改善 |

**总结**: Faithfulness 0.38 → **0.6353**（↑67%），瓶颈已收敛。仍存留的低分 query（4 条 F<0.4）均为列举/假设/分析型问题，属于检索增强固有限制。**V3 可收敛，进入 V4 规划。**
