...
### Phase 3：RAGAS 评估（即将开始）🟡

- **操作**: 集成 RAGAS 跑 20 条 query，出 faithfulness + answer_relevancy baseline（即将实施）
- **目的**: 简历上有量化指标，后续优化有数字可对比
- **注意**: 从"纯肉眼判断"转向"指标驱动 + 目测验证"双轨制

### 当前收敛状态

20 条 query 当前质量分布：
- ✅ 稳定正确：15 条（含经三轮修复后从❌/⚠️改善的 3 条）
- ⚠️ 部分偏差：2 条（#10 多用户/#17 大文件）
- ❌ 明显错误：0 条

剩余 2 条 ⚠️ 的根因是"知识库没写这些局限性"，不是检索质量问题。评估体系到位后以此作为 baseline，逐项排查优化方向。
