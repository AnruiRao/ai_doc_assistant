...
### 2026-06-26：实现 `_merge_short_chunks()`

- `src/ingestion/chunker.py` 新增静态方法 `_merge_short_chunks()`
- `recursive_split` 末尾调用：`return self._merge_short_chunks(chunks, chunk_size)`
- 短碎片（< min_threshold = chunk_size × 0.4）附随大 chunk 输出，不单独成块
- 末尾不足阈值的残余合并到上一个 chunk；全短文档兜底独立成块

### 2026-06-26：修复 buffer 未清空导致 chunk 重叠

**问题**：`_merge_short_chunks` 在累积长度超过 min_threshold 输出 chunk 后，只重置了 `buf_len = 0`，但忘记清空 `buffer = []`。导致后续写操作继续 `append` 到旧 buffer 上，每次输出的 chunk 都包含之前所有内容，形成嵌套重叠（三个 chunk 均以相同 frontmatter 开头）。
